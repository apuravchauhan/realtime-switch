import { EventManager, ProvidersEvent, Providers, ClientEventsExtractor } from '@realtime-switch/core';
...
export class SessionManager extends EventManager {
  ...
  private sessionPersistence: FilePersistence;
  ...
  constructor(apiStyle: Providers, sessionId: string) {
    ...
    this.sessionPersistence = new FilePersistence({ basePath: './' });
  ...
  private async mergeSessionConfiguration(event: ProvidersEvent): Promise<void> {
    ...
    await this.saveSessionConfiguration();
  }
